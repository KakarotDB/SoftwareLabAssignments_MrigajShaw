...
namespace BankingSystem {
	...
	class BankAccount {
		...
		public:
			BankAccount(std::string name, int accountNumber, double currentBalance, BankManager &manager);
		...
		public:
			void display(BankAccount account) const {
				std::cout << "Name : " << account.m_name << std::endl
					...
					<< "Current Balance : " << account.m_currentBalance << std::endl;
			}
	...
	BankAccount::BankAccount(std::string name, int accountNumber, double currentBalance, BankManager &manager): m_name(name), m_accountNumber(accountNumber) {
		manager.addAccount();
		if(currentBalance <= 0) m_currentBalance = 0;
		else m_currentBalance = currentBalance;
	}
