...
SELECT crs_cd, MIN(marks) AS min_marks, MAX(marks) AS max_marks, AVG(marks) AS avg_marks
FROM crs_regd
GROUP BY crs_cd;

-- Q3: Total credits of courses registered by a student
SELECT r.crs_rollno, SUM(o.crs_credits) AS total_credits
FROM crs_regd r
JOIN crs_offrd o ON r.crs_cd = o.crs_code
GROUP BY r.crs_rollno;

-- Q4: Number of students in each hostel whose department is CSE
SELECT hostel, COUNT(*) AS num_students
FROM students
WHERE TRIM(deptcode) = 'CSE'
GROUP BY hostel;
...
FROM (
    SELECT hostel, rollno, parent_inc,
           RANK() OVER(PARTITION BY hostel ORDER BY parent_inc DESC) as rank
    FROM students
...
SELECT r.crs_rollno, r.crs_cd, r.marks
FROM crs_regd r
WHERE TRIM(r.crs_cd) IN ('CH103', 'PH106')
  AND r.marks > (
      SELECT r2.marks
      FROM crs_regd r2
      WHERE r2.crs_rollno = 92005102 AND r2.crs_cd = r.crs_cd
  );
